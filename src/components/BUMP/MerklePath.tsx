...
export const MerklePath: FC<ItemValuePathProps<BUMP>> = ({ value, path, idx }) => {
  return (
    <ValueContainer label={`BUMPs[${idx}]`} value={value} path={[...path, idx.toString()]}>
...
const Path: FC<ValuePathProps<Value<BUMPLevel>[]>> = ({ value, path }) => {
  ...
  return (
    <ValueContainer label="Path" value={value} path={pathOfPath}>
      {value.value.map((it, i) => (
        <PathLeaves key={i} idx={i} value={it} path={[...pathOfPath, i.toString()]} />
      ))}
    </ValueContainer>
  );
};
...
const PathLeaf: FC<ItemValuePathProps<BUMPLeaf>> = ({ value, path, idx }) => {
  return (
    <ValueContainer label={`Leaves[${idx}]`} value={value} path={path}>
